...
3. 密级：文档密级（`public` / `internal` / `confidential`）与 ACL 同为召回查询中的**与条件**，
   只能收紧可见性、不能放大：机密文档需 `document.read.confidential`（等级 ≥ `auditor`），
   即使它同时被标为 `public` 或已写进 ACL；无法识别的密级值失败关闭。
4. 输入：限制长度、文件类型与请求频率；检测提示词注入和凭据内容。
5. 输出：只允许返回授权证据，敏感字段按策略遮盖，无证据时拒答。
6. 网络：查询服务只访问数据库、对象存储和批准的模型端点。
7. 密钥：通过企业密钥管理系统注入，不写入仓库、日志或状态接口。
8. 审计：管理操作全量记录；查询日志默认不保存问题与回答正文。
