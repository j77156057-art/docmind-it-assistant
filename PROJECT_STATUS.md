...
- 异步索引：`ingestion_jobs` 业务队列、`worker` 进程（抢占、心跳、僵尸回收、可重试性分类）、导入接口 `202 + job_id`、任务重试/取消、队列健康诊断；`reindex`/`withdraw`/`evaluate` 任务类型已由 Worker 实现（reindex 重置版本为 `queued` 后重嵌、withdraw 走既有治理方法并写审计、evaluate 异步跑黄金集并把门禁结论写进评测运行记录），可重试失败通过行内 `next_attempt_at` 持久化指数退避（迁移 `20260922_0011`），避免坏上游被紧循环打爆。
...
在 CI 里按 SKIPPED 记录，因此不会以 0 分污染指标。CI 真 PG 实测（PG pgvector + tsvector RRF 路径）：

| run | recall@5 | 引用命中率（strict） | 通过 |
|---|---|---|---|
| `35816968938` / `35817447640` / `35823201964`（IDF 修正前） | 0.8571 | 0.7143 / 0.7619 / 0.7619 | 16/21 |
| **`35826053179`（IDF 修正后）** | **1.0** | **0.9524**（0.7619） | **20/21** |

修正前后都与本地 SQLite 同量级，且两条检索路径（SQLite portable 全表扫描 与 PG RRF）在这份语料上
给出同一结论——即这个排序缺陷与后端无关。

⚠️ **已知抖动**：修正前同一语料、同一配置的连续两次 CI 之间有 1 题翻转（`passed_cases` 15 → 16，
引用命中率 0.7143 → 0.7619）。修正后余量大幅拉开（`recall@5 1.0` vs 阈值 0.8、引用 0.9524 vs 0.5），
剩余 1 道失败题不再逼近阈值。若日后仍出现偶发红，先查那 1 题的排序不确定性，而不要直接调阈值。
...
7. **已收口**：`evaluate` 异步评测任务类型（Worker 已实现，见「已完成」；入队入口 `POST /api/admin/evaluation/runs` + `"queue": true`）。仍未做的是**发布路径改消费异步评测结论**（现仍在发布请求内同步整跑），那是需要拍板的取舍，见 README「当前限制」。
